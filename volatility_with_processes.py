...
class TikVolatility:
    ...
    def _csv_handler(self, dirpath, file):
        csv_thread = CsvMultyProcess(dirpath, file, self.collector)
        csv_thread.start()
        csv_thread.join()
        while not self.collector.empty():
            data = self.collector.get()
            self.tik_list.append(data)
...
class CsvMultyProcess(multiprocessing.Process):

    def __init__(self, dirpath, file, collector, *args, **kwargs):
    ...
    def run(self):
        full_path = os.path.join(self.dirpath, self.file)
        ...
        self.collector.put([csv_list[0][0], volatylity])
